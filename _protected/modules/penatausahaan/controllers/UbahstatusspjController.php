 ...
 */
class UbahstatusspjController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    public function actionIndex()
    {
        ...
        $dataProvider->query->andWhere(['tahun' => $Tahun])->andWhere('kd_sah > 1')->andWhere('(tahun, sekolah_id, no_spj) NOT IN (SELECT tahun, sekolah_id, no_spj FROM ta_sp3b_rinc WHERE tahun = :tahun)', [':tahun' =>$Tahun]);
    ...
    public function actionUpdate($tahun, $no_spj)
    {
        ...
        $model = $this->findModel($tahun, $no_spj);
        IF(\app\models\TaSP3BRinc::findOne(['tahun' => $model->tahun, 'no_spj' => $model->no_spj, 'sekolah_id' => $model->sekolah_id])){
            Yii::$app->getSession()->setFlash('warning',  'SPJ ini sudah diproses di SP3B, tidak dapat diubah atau dihapus.');
            return $this->redirect(Yii::$app->request->referrer);
        }        
    ...
    protected function cekakses(){
        ...
        IF(Yii::$app->user->identity){
            $akses = \app\models\RefUserMenu::find()->where(['kd_user' => Yii::$app->user->identity->kd_user, 'menu' => 510])->one();
            IF($akses){
                return true;
            }else{
                return false;
