...
$pdf->MultiCell(150,5,'REKAPITULASI REKOMENDASI PEMBAYARAN HUTANG
...
$pdf->Cell($w['1'],11,'NOMOR RPH','LTR',0,'C');
...
foreach($model as $model){
	...
	IF($y2 > 196 || $y1 + (5*(strlen($model['No_RPH'])/35)) > 196 ){ //cek pagebreak
	...
	$pdf->MultiCell($w['1'],6,$model['No_RPH'],'T','L');
	$xcurrent = $xcurrent+$w['1'];
 	$pdf->SetXY($xcurrent, $y);
	$pdf->MultiCell($w['2'],6,$model['noSPH']['No_Kontrak'],'T','L');
	$xcurrent = $xcurrent+$w['2'];
	$y1 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
	$pdf->SetXY($xcurrent, $y);
	$pdf->MultiCell($w['3'],6,date('d-m-Y', strtotime($model['noSPH']['Tgl_Kontrak'])),'T','C');
	$xcurrent = $xcurrent+$w['3'];
	$pdf->SetXY($xcurrent, $y);
	$pdf->MultiCell($w['4'],6,$model['noSPH']['Pekerjaan'],'T','L');
	$y2 = $pdf->GetY(); //berikan nilai untuk $y1 titik terbawah Uraian Kegiatan
	$xcurrent = $xcurrent+$w['4'];
	$pdf->SetXY($xcurrent, $y);
	$pdf->MultiCell($w['5'],6,$model['noSPH']['Nm_Perusahaan'],'T','L');
	$xcurrent = $xcurrent+$w['5'];
	$pdf->SetXY($xcurrent, $y);
	$pdf->MultiCell($w['6'],6,number_format($model['Nilai_Bayar'],0,',','.'),'T','R');
	$xcurrent = $xcurrent+$w['6'];
	$pdf->SetXY($xcurrent, $y);

	
	$totalhutang = $totalhutang+$model['Nilai_Bayar'];
...
$pdf->SetFont('Arial','B',10);
$pdf->MultiCell(255,5,'Pangkalan Balai, '.DATE('d').' '.bulan(DATE('m')).' '.DATE('Y'), '', 'J', 0);
$pdf->SetXY(255,$pdf->gety());
$pdf->SetFont('Arial','B',10);
$pdf->MultiCell(255,5,'Kepala SKPD', '', 'j', 0);
$pdf->SetXY(255,$pdf->gety()+20);
$pdf->SetFont('Arial','B',10);
$pdf->MultiCell(255,5,$kepala, '', 'j', 0);
